...
most_popular_in_year <- function(year){
  new_table <- paste("name_", year, sep = "")
  new_table <- data.frame(names[names$year == year, ])
  new_table[new_table$prop == max(new_table$prop), "name"]
}
...
number_in_million <- function(name, year){
  new_table <- paste("name_", year, sep = "")
  new_table <- data.frame(names[names$year == year, ])
  goal <- new_table[new_table$name == name, "prop"]
  print(goal * 1000000)
}
